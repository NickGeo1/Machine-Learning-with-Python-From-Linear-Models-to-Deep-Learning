...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Your code here
    loss_sum = 0
    for idx,ftr_vec in enumerate(np.array(feature_matrix)):
        loss_sum += hinge_loss_single(ftr_vec, labels[idx], theta, theta_0)
    return (1/feature_matrix.shape[0]) * loss_sum
...
def perceptron_single_step_update(
    ...
    return (current_theta + label*feature_vector, current_theta_0 + label) if value <= 0 else (current_theta, current_theta_0)
...
def perceptron(feature_matrix, labels, T):
    ...
    curr_theta = np.zeros(shape=data.shape[1])
    curr_theta_0 = 0

    for t in range(T):
        for i in get_order(data.shape[0]):
            # Your code here
            curr_theta, curr_theta_0 = perceptron_single_step_update(data[i], labels[i], curr_theta, curr_theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    curr_theta = np.zeros(shape=data.shape[1])
    curr_theta_0 = 0
    theta_sum = np.zeros(shape=data.shape[1])
    theta_0_sum = 0

    for t in range(T):
        for i in get_order(data.shape[0]):
            curr_theta, curr_theta_0 = perceptron_single_step_update(data[i], labels[i], curr_theta, curr_theta_0)
            theta_sum += curr_theta
            theta_0_sum += curr_theta_0

    return ( (1/(data.shape[0]*T))*theta_sum, theta_0_sum/(data.shape[0]*T) )
...
def pegasos_single_step_update(
    ...
    # Your code here
    value = label * (np.dot(feature_vector, theta) + theta_0)
    return ((1 - eta * L) * theta + eta * label * feature_vector, theta_0 + eta*label) if value <= 1 else ((1 - eta * L) * theta, theta_0)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(data.shape[0]):
            step += 1
            eta_t = 1/np.sqrt(step)
            curr_theta, curr_theta_0 = pegasos_single_step_update(data[i], labels[i], L, eta_t, curr_theta, curr_theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    # Your code here
    data = np.array(feature_matrix)
    predictions = (data * theta).sum(axis = 1) + theta_0
