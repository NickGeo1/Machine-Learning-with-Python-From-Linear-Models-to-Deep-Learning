...
def epsilon_greedy(state_vector, epsilon):
    ...
    if random_action:
        ...
        with torch.no_grad():
            action_vec, object_vec = model.forward(state_vector)
        action_index, object_index = action_vec.argmax().item(), object_vec.argmax().item()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    # TODO Your code here
    if not terminal:
        loss = (1/2) * (reward + GAMMA*maxq_next - ( q_value_cur_state[0][action_index] + q_value_cur_state[1][object_index]) / 2) ** 2
    else:
        loss = (1 / 2) * (reward - ( q_value_cur_state[0][action_index] + q_value_cur_state[1][object_index]) / 2) ** 2
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if for_training:
            # update Q-function.
            # TODO Your code here
            new_state_vector = torch.FloatTensor(utils.extract_bow_feature_vector(next_room_desc + next_quest_desc, dictionary) )
            deep_q_learning(current_state_vector, action_idx, object_idx, reward, new_state_vector, terminal)
