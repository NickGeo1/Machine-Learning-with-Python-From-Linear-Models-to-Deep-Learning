...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    phi[tuple2index(action_index, object_index)] = current_state_vector.T
    if not terminal:
        theta += ALPHA*(reward + GAMMA*(theta @ next_state_vector).max() - (theta @ current_state_vector)[tuple2index(action_index, object_index)] )*phi # TODO Your update here
    else:
        theta += ALPHA*(reward - (theta @ current_state_vector)[tuple2index(action_index, object_index)] )*phi
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        state_vector = utils.extract_bow_feature_vector(current_room_desc + current_quest_desc, dictionary)
        action_idx, object_idx = epsilon_greedy(state_vector, theta, epsilon)

        next_room_desc, next_quest_desc, reward, terminal = \
            framework.step_game(current_room_desc, current_quest_desc, action_idx, object_idx)

        if for_training:
            # update Q-function.
            # TODO Your code here
            new_state_vector = utils.extract_bow_feature_vector(next_room_desc + next_quest_desc, dictionary)
            linear_q_learning(theta, state_vector, action_idx, object_idx, reward, new_state_vector, terminal)
