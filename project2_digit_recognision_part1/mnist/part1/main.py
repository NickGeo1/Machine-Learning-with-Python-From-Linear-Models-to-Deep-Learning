...
theta, _ = softmax_regression(train_pca, train_y, temp_parameter=1, alpha=0.3, lambda_factor=1.0e-4,
                                                      k=10, num_iterations=150)
test_error_pca = compute_test_error(test_pca, test_y, theta, temp_parameter=1)
print(f"test error pca 18 = {test_error_pca}")
...
theta, _ = softmax_regression(train_cube, train_y, temp_parameter=1, alpha=0.3, lambda_factor=1.0e-4,
                                                      k=10, num_iterations=150)
test_error_pca10_cube = compute_test_error(test_cube, test_y, theta, temp_parameter=1)
print(f"test error pca 10 cube = {test_error_pca10_cube}")'''
...
predictions = svm.predict(test_pca10)
print(f"test_error_pca10_cube_sckit_svm: {1 - np.mean(predictions == test_y)}")'''

# Rbf svm using scikit-learn
svm = SVC(random_state=0, kernel="rbf", degree=3)
svm.fit(train_pca10, train_y)
predictions = svm.predict(test_pca10)
print(f"test_error_pca10_rbf_sckit_svm: {1 - np.mean(predictions == test_y)}")